...
for (let character_geometry_index = 0; character_geometry_index < 6; ++character_geometry_index) {
    ...
    const cube = new THREE.Mesh(character_geometry[character_geometry_index], material);
...
function MakeCharacterGeometry(character) {
    character_geometry.push(MakeBox(4, 12, 4, 'left_leg'));
    character_geometry.push(MakeBox(4, 12, 4, 'right_leg'));
    character_geometry.push(MakeBox(8, 12, 4, 'body'));
    character_geometry.push(MakeBox(4, 12, 4, 'left_arm'));
    character_geometry.push(MakeBox(4, 12, 4, 'right_arm'));
    character_geometry.push(MakeBox(8, 8, 8, 'head'));
}
//buffergeometry를 통한 custom mesh 생성
function MakeBox(nx, ny, nz, character_component) {
